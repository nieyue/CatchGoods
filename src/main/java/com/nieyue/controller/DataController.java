...
public class DataController {
	...
	public @ResponseBody StateResultList browsePagingData(
			...
			list= dataService.browsePagingData(createDate,bookId,acountId,pageNum, pageSize, orderName, orderWay);
			if(list.size()>0){
	...
	public @ResponseBody StateResultList statisticsData(
			...
			@RequestParam(value="orderName",required=false,defaultValue="data_id") String orderName,
			@RequestParam(value="orderWay",required=false,defaultValue="desc") String orderWay,HttpSession session)  {
		List<Data> list= dataService.statisticsData(startDate,endDate,bookId,acountId,pageNum, pageSize, orderName, orderWay);
		if(list.size()>0){
			return ResultUtil.getSlefSRSuccessList(list);
	...
	public @ResponseBody int countAll(
			...
			@RequestParam(value="acountId",required=false)Integer acountId,
			HttpSession session)  {
		int count = dataService.countAll(createDate,bookId,acountId);
		return count;
	}
	/**
	 * 数据单个加载
	...
	public @ResponseBody StateResultList loadDataByParams(
			...
			HttpSession session)  {
		List<Data> list = new ArrayList<Data>();
		Data Data = dataService.loadData(dataId,crateDate,bookId,acountId);
		if(Data!=null &&!Data.equals("")){
			list.add(Data);
			return ResultUtil.getSlefSRSuccessList(list);
		}else{
